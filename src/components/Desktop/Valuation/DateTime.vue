        ...
        <div class="w-9 mx-auto border-round-3xl shadow-2 px-4 mt-2">
            <div class="flex flex-column align-items-center">
                ...
                <div class="my-4 w-full">
                    <div class="flex custom-border-radius align-items-center custom-dark-gray-border border-1 px-4">
                        <Calendar class="w-full h-4rem align-items-center gap-3 custom-calendar-input custom-dark-calendar pr-2" placeholder="Choose date"
                            :showWeek="false" v-model="valuationDate" dateFormat="dd/mm/yy" :minDate="minDate" @dateSelect="changeDate()" />
                        <i class="fas fa-calendar-days text-2xl custom-gray"></i>
                    </div>

                    <Dropdown v-model="valuationTime" :options="timesArray" optionLabel="name" optionValue="name" placeholder="Choose time"
                        class="w-full h-4rem align-items-center custom-border-radius font-bold mt-4 pl-4 pr-3 
                            custom-dark-dropdown-1 custom-dropdown-1 border-1"  @change="changeTime()" />
...
const showPlacesModal = () => {
    if(valuationDate.value != null && valuationTime.value != null) {
        let formattedDate = typeof valuationDate.value != "string" ? valuationDate.value.toLocaleDateString() : valuationDate.value
