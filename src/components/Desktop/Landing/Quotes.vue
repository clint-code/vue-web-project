<template>
    <div class="custom-desktop-view custom-width-80"
        :class="{ 'mt-3': props.insuranceClass == 'PRIVATE', 'mt-7': props.insuranceClass != 'PRIVATE' }">
                        ...
                        <div class="custom-card border-round-2xl relative p-3">
                            ...
                            <div class="flex justify-content-between">
                                ...
                                <template v-if="quote.LipaFullAmount != null">
                                    <label class="font-bold text-right w-5">KES {{ quote.LipaFullAmount.toLocaleString()
                                        }}</label>
                                </template>
                            </div>
                            <hr class="custom-gray" />
                            ...
                            <div class="flex justify-content-between">
                                ...
                                <template v-if="quote.LipaPolePoleDeposit != null">
                                    <label class="font-bold text-right w-6">KES {{
            quote.LipaPolePoleDeposit.toLocaleString() }}</label>
                                </template>
                            </div>
                                                ...
                                                <template v-if="formattedBenefits != null">
                                                    <div v-for="(benefit, index) in formattedBenefits">
                                                        <div class="flex justify-content-between align-items-center custom-gray-border-bottom py-2">
                                                            <div class="flex align-items-center">
                                                                <Checkbox v-model="benefit.active" :binary="true" variant="filled" @change="changeBenefit(benefit, index)" />
                                                            ...
                                                            <template v-if="benefit.limitInput && benefit.active">
                                                                <div class="w-4">
                                                                    <InputGroup class="border-round-2xl">
                                                                        ...
                                                                        <InputText
                                                                            class="border-y-2 border-x-none custom-dark-gray-border" v-model="benefit.benefitValue"
                                                                            placeholder="Estimate value" size="small" />

                                                                        <InputGroupAddon class="border-round-right-2xl custom-box-shadow border-none custom-dark-gray-bg-1" @click="changeBenefitWithLimit(benefit)">
                                        ...
                                        <TabPanel>
                                            ...
                                            <div class="w-full border-round-2xl bg-yellow-500 border-1 border-yellow-500 px-3 py-2 px-1 mt-4"
                                                ...
                                                <div class="flex justify-content-between align-items-center">
                                                    ...
                                                    <template v-if="quote.LipaFullAmount != null">
                                                        <label class="font-bold text-right">KES {{ quote.LipaFullAmount.toLocaleString() }}</label>
                                                    </template>
...
const changeBenefitWithLimit = (benefit) => {
    if(benefit.benefitValue > benefit.minimumLimit && benefit.benefitValue < benefit.maximumLimit) {
        ...
        showErrorToast("Error", "Benefit value should be greater than " + benefit.minimumLimit.toLocaleString() +
        " but less than " + benefit.maximumLimit.toLocaleString())
    }
}
...
const addOrRemoveBenefit = (data) => {
    ...
    quoteService.addOrRemoveBenefit(data)
        .then((response) => {
